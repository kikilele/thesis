...
sink.x=0.5*xm;
sink.y=0.5*ym;
plot(sink.x,sink.y,'r+');    %中心位置

%Number of Nodes in the field
n = 100;
%Optimal Election Probability of a node to become cluster head
p=0.05;         %%这样是改进的LEACH算法，为了控制每次在cluster中出现的HN的数目
        ...
        if(S(i).E>0)
            ...
            if ((S(i).G)<=0) 
                ...
                if(temp_rand<=(p/(1-p*mod(r,round(1/p)))))  
                    ...
                    if (distanceBroad >=do)
                        S(i).E = S(i).E-(ETX*ctrPacketLength + Emp*ctrPacketLength*(distanceBroad*distanceBroad*distanceBroad*distanceBroad));%广播自成为簇头？？？？？为什么这里是4次方
                    else
