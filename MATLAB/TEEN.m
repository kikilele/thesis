...
Efs=10*0.000000000001;Emp=0.0013*0.000000000001;
...
S(n+1).xd=sink.x;S(n+1).yd=sink.y;plot(S(n+1).xd,S(n+1).yd,'x');
...
countCHs;rcountCHs=rcountCHs+countCHs;flag_first_dead=0;
...
countCHs=0;cluster=1;for i=1:1:n
   if(S(i).E>0)
   temp_rand=rand;     
   if ( (S(i).G)<=0)
...
STATISTICS(r+1).CLUSTERHEADS=cluster-1;CLUSTERHS(r+1)=cluster-1;
...
countCHs;rcountCHs=rcountCHs+countCHs;
