...
for r2=0:1:rmax
    ...
    for i=1:length(clCopy)
        if S2(i).E>0 && S2(i).type=='N'
            temp_rand2=rand;
            if S2(i).G<=0
                if( temp_rand2 <= (p2/(1-p2*mod(r2,round(1/p2)))))
                    ...
                    S2(i).type='C';
                    S2(i).G = round(1/p2)-1;
                    CL2(cluster2).xd = S2(i).xd;
                    CL2(cluster2).yd = S2(i).yd;
                    distance2=sqrt( (S2(i).xd-(S2(iclCopy(clCopy(i))).xd) )^2 + (S2(i).yd-(S2(iclCopy(clCopy(i))).yd) )^2 );%到leader的距离
                    ...
                    if (distanceBroad2 > do)
                        S2(i).E = S2(i).E- ( ETX * ctrPacketLength + Emp* ctrPacketLength*( distanceBroad2*distanceBroad2*distanceBroad2*distanceBroad2 ));
                    else
                        S2(i).E = S2(i).E- ( ETX * ctrPacketLength + Efs * ctrPacketLength*( distanceBroad2*distanceBroad2));
                    end
                    
                    distance2;%CH到Leader的距离,CH自身发送数据和leader接收消耗
                    if (distance2 > 0)
                        S2(iclCopy(clCopy(i))).E = S2(iclCopy(clCopy(i))).E - ( (ERX + EDA)*packetLength );                          
                    end
                    if (distance2 > do)                                              
        ...
        if S2(i).type=='N' && S2(i).E>0
            ...
            if(cluster2 -1 >= 1)%如果有簇头存在
                ...
                for c = 1:1:cluster2 - 1 %簇头数量一共是cluster - 1
                    ...
                    temp2 = sqrt( (S2(i).xd - CL2(c).xd)^2 + (S2(i).yd - CL2(c).yd)^2 );
                    if ( temp2 < min_dis2 )
                        min_dis2 = temp2;
                        min_dis_cluster2 = c;
                    end
                    %接收簇头发来的广播的消耗
                    S2(i).E = S2(i).E - ETX * ctrPacketLength;
                end
                
                %Energy dissipated by associated Cluster Head普通节点发送数据包到簇头消耗,和加入消息
                min_dis2;
                if (min_dis2 > do)
                    S2(i).E = S2(i).E - ( ETX*(ctrPacketLength) + Emp * ctrPacketLength*( min_dis2 * min_dis2 * min_dis2 * min_dis2)); %向簇头发送加入控制消息
                    S2(i).E = S2(i).E - ( ETX*(packetLength) + Emp*packetLength*( min_dis2 * min_dis2 * min_dis2 * min_dis2)); %向簇头数据包
                else
                    S2(i).E = S2(i).E - ( ETX*(ctrPacketLength) + Efs*ctrPacketLength*( min_dis2 * min_dis2)); %向簇头发送加入控制消息
                    S2(i).E = S2(i).E - ( ETX*(packetLength) + Efs*packetLength*( min_dis2 * min_dis2)); %向簇头数据包
                end
                S2(i).E = S2(i).E - ETX*(ctrPacketLength); %接收簇头确认加入控制消息
                ...
                S2(i).min_dis = min_dis2;
                S2(i).min_dis_cluster = min_dis_cluster2;
            else
                min_dis2=sqrt( (S2(i).xd - S2(iclCopy(clCopy(i))).xd)^2 + (S2(i).yd - S2(iclCopy(clCopy(i))).yd)^2 );
                if (min_dis2>do)
                    S2(i).E=S2(i).E- ( ETX*(packetLength) + Emp*packetLength*( min_dis2 * min_dis2 * min_dis2 * min_dis2));
                end
                if (min_dis2<=do)
                    S2(i).E=S2(i).E- ( ETX*(packetLength) + Efs*packetLength*( min_dis2 * min_dis2));
                end
                packets_TO_BS2=packets_TO_BS2+1;
            end
        end
...
subplot(2,1,1);
plot(x11,y11,'r--',x21,y21,'b--',x31,y31,'g--',x51,y51,'k--');
...
subplot(2,1,2);
plot(x12,y12,'r--',x22,y22,'b--',x32,y32,'g--',x52,y52,'k--');
...
plot(x13,y13,'r--',x23,y23,'g--',x33,y33,'b--',x53,y53,'y--');
hold on;
title('Throughput');
xlabel('Round');
